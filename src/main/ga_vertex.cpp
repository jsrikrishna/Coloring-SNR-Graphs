...
vector< pair<vector<int>,int> > COMPUTE(vector< vector<int> > adj,vector< vector<int> > pop,int n,int m,int no_pop)
{
...
//	srand((int)time(NULL));
    ...
    for(int i=0;i<n;i++)
    {
            ...
            if(adj_connect< min_degree)
            min_degree=adj_connect;
            cout<<endl;
            adj.push_back(a);
            a.clear();
    }
    ...
    cout<<"the minimum degree is "<<min_degree<<endl;
    cout<<"max connectivity is "<<max_connect<<endl;
    ...
    while(gen<=generation && isTerminate==false)
    {
                   ...
                   fitness=COMPUTE(adj,pop,n,m,no_pop);
                   if(!isTerminate)
                   {
                       selection=SELECT(fitness,n,m,no_pop);
                       crossover=CROSSOVER(selection,n,m,cross_rate,0.6,adj);
